...
public class MessagingManager {
	...
	public static void reload() throws NeoIOException {
		...
		BungeeCore.loadFiles(new File(BungeeCore.folder(), "messages"), (cfg, file) -> {
			for (String key : cfg.getKeys()) {
				Config sec = cfg.getSection(key);
				messages.put(key.toUpperCase(), parsePage(sec));
			}
		});
	}
	
	public static void sendMessage(CommandSource s, CommandSource recipient, String key) {
		sendMessage(s, recipient, key, 1);
	}
	
	public static void sendMessage(CommandSource s, CommandSource recipient, String key, int page) {
		page--;
		if (messages.containsKey(key.toUpperCase())) {
			...
			BungeeCore.logger().warning("[NeoCore] Failed to send message to " + s.getName() + ", key doesn't exist: " + key);
		}
	}
	
	public static BaseComponent[][] parsePage(Config cfg) {
		BaseComponent[][] list;
		if (cfg.contains("pages")) {
			Config pages = cfg.getSection("pages");
	...
	public static BaseComponent[] parseMessage(Config cfg) {
