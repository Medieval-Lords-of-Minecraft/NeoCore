...
public class CmdSilentMutableBroadcast implements SimpleCommand {
	private static final Component usage = Component.text("/smbc [tag for mute] [broadcast msg]", NamedTextColor.RED);
	@Override
	public void execute(Invocation inv) {
		if (inv.arguments().length < 2) {
			...
			Util.mutableBroadcast(inv.arguments()[0], Component.text(SharedUtil.connectArgs(inv.arguments(), 1)), false);
		}
	}
